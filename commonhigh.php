...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM highestjobs WHERE high_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['high_title'];
            $salary = $row['high_salary'];
            $company_name = $row['high_company'];
            $description = $row['high_descrip'];
            $img = $row['high_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['high_company'] ?>'>Apply</a></button>
            </div>
        </section>
