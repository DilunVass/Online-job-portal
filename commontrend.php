...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM trendjobs WHERE trend_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['trend_title'];
            $salary = $row['trend_salary'];
            $company_name = $row['trend_company'];
            $description = $row['trend_descrip'];
            $img = $row['trend_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['trend_company'] ?>'>Apply</a></button>
            </div>
        </section>
