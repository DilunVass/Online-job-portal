...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM topjobs WHERE top_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['top_title'];
            $salary = $row['top_salary'];
            $company_name = $row['top_company'];
            $description = $row['top_descrip'];
            $img = $row['top_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['top_company'] ?>'>Apply</a></button>
            </div>
        </section>
