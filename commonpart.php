...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM parttimejobs WHERE part_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['part_title'];
            $salary = $row['part_salary'];
            $company_name = $row['part_company'];
            $description = $row['part_descrip'];
            $img = $row['part_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['part_company'] ?>'>Apply</a></button>
            </div>
        </section>
