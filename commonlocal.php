...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM localjobs WHERE local_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['local_title'];
            $salary = $row['local_salary'];
            $company_name = $row['local_company'];
            $description = $row['local_descrip'];
            $img = $row['local_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['local_company'] ?>'>Apply</a></button>
            </div>
        </section>
