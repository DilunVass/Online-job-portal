...
<body>
    ...
    <?php
        if(isset($_GET['id'])){
            $jobID = $_GET['id'];
            $sql = "SELECT * FROM overseasjobs WHERE overseas_ID = '$jobID'";
            $result = mysqli_query($conn,$sql);

            $row = mysqli_fetch_assoc($result);

            $title = $row['overseas_title'];
            $salary = $row['overseas_salary'];
            $company_name = $row['overseas_company'];
            $description = $row['overseas_descrip'];
            $img = $row['overseas_img'];
        }
    ?>
    <!---apply jobs part-->
    ...
    <section class="topjobs_box1">
            <div class="topjobs_box1_sub">
                ...
                <button class="top_wind_mini_bt"><a href='Jobapply.php?id=<?php echo $row['overseas_company'] ?>'>Apply</a></button>
            </div>
        </section>
